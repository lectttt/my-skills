...
graph TD
    ...
    Agents["Universal 目录 (~/.agents/skills/)"]
    Gemini["Antigravity 目录 (~/.gemini/antigravity/skills/)"]
    
    Repo -- "ln -s 或 cp (拷贝)" --> Agents
    Agents -- "ln -s 或 cp" --> Gemini
    
    User["用户在 IDE 修改代码"] -- "直接修改" --> Gemini
    Gemini -- "同步回传" --> Repo
    SyncSkill["skill-git-sync"] -- "执行 robust push" --> GitHub["GitHub 云端"]
```

> [!NOTE]
> 这种“三位一体”的结构确保了开发灵活性。在某些环境下，若软链接（ln -s）因权限或识别问题失效，可改用**实体目录拷贝（cp）**，此技能依然能完美支持同步。
